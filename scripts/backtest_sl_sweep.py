#!/usr/bin/env python3
"""
90-Day Portfolio Backtest: 0.3R TP / Variable SL Sweep
Focus 5 assets: BTC, ETH, SOL, SUI, ADA
Both tiers: 1H scalp + 4H swing
Simulated $1,000 portfolio with 2% risk per trade

Sweeps 5 SL levels (0.4R, 0.5R, 0.6R, 0.7R, 0.8R) all with 0.3R TP.
Position sizing based on ORIGINAL 1R stop loss distance.
MIN_RR = 1.0 for all scenarios.

Usage:
    python3 scripts/backtest_sl_sweep.py
"""
...
# TP fraction (same for all scenarios)
TP_FRACTION = 0.3   # 30% of entry-to-T1 distance

# SL fractions to sweep
SL_FRACTIONS = [0.4, 0.5, 0.6, 0.7, 0.8]
...
class Signal:
    ...
    risk_1r: float         # Original 1R distance
    rr_ratio: float
...
# ─── Signal Resolution — 0.3R TP / Variable SL ─────────────────────────────

def make_resolve_fn(sl_fraction):
    """Create a resolution function for a given SL fraction."""
...
# ─── Signal Generation (returns raw signal data, not yet resolved) ──────────

def generate_signals(candles_cache):
    """
    Run the full pipeline to generate signals. Returns a list of signal
    'templates' (dicts) that can be instantiated for each SL scenario.
    """
    ...
    # Track which zones have been signaled (to prevent duplicates within a run)
    signaled_zones = set()
        ...
        for asset in ASSETS:
            ...
            for tier_name in ["scalp", "swing"]:
                ...
                for zone in zones:
                    ...
                    # Dedup: check if this zone already signaled
...
# ─── Portfolio-Aware Backtest Runner (uses pre-generated signals) ───────────

def run_portfolio_backtest(candles_cache, signal_templates, sl_fraction):
    """
    Run backtest with a specific SL fraction using pre-generated signal templates.
    """
    resolve_fn = make_resolve_fn(sl_fraction)
    portfolio_value = STARTING_CAPITAL
    peak_value = STARTING_CAPITAL
    max_drawdown_pct = 0.0

    # Create Signal objects from templates, sorted by entry_time
    all_signals = []
    ...
    # Build timeline from 1h candles
    ref_symbol = ASSETS[0]["symbol"]
    ref_candles = candles_cache[ref_symbol]["1h"]
    cutoff = datetime.now(timezone.utc) - timedelta(days=BACKTEST_DAYS)
    ...
    for ref_candle in ref_candles:
        ...
        # Add new signals that enter at this time
...
def main():
    print("=" * 120)
    print("90-DAY PORTFOLIO BACKTEST: 0.3R TP / STOP LOSS SWEEP")
    print(f"Assets: {', '.join(a['ticker'] for a in ASSETS)}")
    print(f"Tiers: scalp (1H/4H) + swing (4H/1D)")
    print(f"Score floor: {MIN_SCORE} | MIN_RR: {MIN_RR} | Period: {BACKTEST_DAYS} days")
    print(f"Starting capital: ${STARTING_CAPITAL:,.0f} | Risk per trade: {RISK_PER_TRADE_PCT}%")
    print()
    print("Risk Model:")
    print(f"  TP = {TP_FRACTION:.0%} of entry-to-T1 distance (0.3R)")
    print(f"  SL sweep: {', '.join(f'{sl}R' for sl in SL_FRACTIONS)}")
    print("  Position sized on original 1R SL (2% of portfolio = 1R)")
    print()
    print("  SL Level | Win P&L  | Loss P&L | Breakeven WR")
    print("  " + "─" * 50)
    breakeven_wrs = {}
    for sl in SL_FRACTIONS:
        win_pnl = TP_FRACTION * RISK_PER_TRADE_PCT
        loss_pnl = sl * RISK_PER_TRADE_PCT
        be_wr = (sl / (TP_FRACTION + sl)) * 100
        breakeven_wrs[sl] = be_wr
        print(f"  {sl}R SL    | +{win_pnl:.1f}%    | -{loss_pnl:.1f}%    | {be_wr:.1f}%")
    print("=" * 120)
    ...
    # Generate signals once (same for all SL scenarios)
    print("\nGenerating signals (shared across all SL scenarios)...")
    signal_templates = generate_signals(candles_cache)
    print(f"  Generated {len(signal_templates)} signal templates")

    # Run each SL scenario
    results = {}
    for sl_frac in SL_FRACTIONS:
        label = f"0.3R TP / {sl_frac}R SL"
        print(f"\n{'='*60}")
        print(f"Running: {label}...")
        print(f"{'='*60}")
        signals, monthly, final_val, dd = run_portfolio_backtest(
            candles_cache, signal_templates, sl_frac
        )
        closed = [s for s in signals if s.status == "closed"]
        print(f"  Complete: {len(closed)} closed signals, final value: ${final_val:,.2f}")
        results[sl_frac] = {
            "signals": signals,
            "monthly": monthly,
            "final": final_val,
    ...
    # ─── Comparison Table ────────────────────────────────────────────────────
    print(f"\n\n{'='*120}")
    print("  STOP LOSS SWEEP COMPARISON TABLE")
    print(f"{'='*120}")
    print()
    print(f"  {'SL Level':<10} {'Signals':>8} {'Win Rate':>9} {'BE WR':>8} {'WR-BE':>8} {'Final $':>10} {'Return %':>10} {'PF':>8} {'Max DD':>8}")
    print(f"  {'─'*90}")

    # Reference row: 1.0R SL from previous test
    print(f"  {'1.0R SL':<10} {'--':>8} {'71.9%':>9} {'23.1%':>8} {'+48.8%':>8} {'$1,025':>10} {'+2.5%':>10} {'1.03':>8} {'--':>8}")

    all_stats = {}
    for sl_frac in SL_FRACTIONS:
        r = results[sl_frac]
        s = compute_stats(r["signals"], r["final"], r["dd"])
        all_stats[sl_frac] = s
        be_wr = breakeven_wrs[sl_frac]
        wr_margin = s["wr"] - be_wr
        margin_str = f"{wr_margin:>+.1f}%"
        print(f"  {sl_frac}R SL{'':<5} {s['total']:>8} {s['wr']:>8.1f}% {be_wr:>7.1f}% {margin_str:>8} ${s['final']:>9,.2f} {s['ret']:>+9.1f}% {s['pf']:>8.2f} {s['dd']:>7.1f}%")
    ...
    for sl_frac in SL_FRACTIONS:
        print(f" {f'{sl_frac}R SL':>14}", end="")
    print()
    sep_len = 25 + 14 * len(SL_FRACTIONS)
    print(f"  {'─' * sep_len}")

    detail_rows = [
        ("Starting Capital", lambda s: f"${STARTING_CAPITAL:,.0f}"),
    ...
    for label, fn in detail_rows:
        ...
        for sl_frac in SL_FRACTIONS:
            print(f" {fn(all_stats[sl_frac]):>14}", end="")
        print()
    ...
    best_sl = max(SL_FRACTIONS, key=lambda sl: all_stats[sl]["final"])
    best = all_stats[best_sl]
    print(f"  BEST BY RETURN: {best_sl}R SL")
    print(f"    Final: ${best['final']:,.2f} ({best['ret']:+.1f}%) | WR: {best['wr']:.1f}% | PF: {best['pf']:.2f} | DD: {best['dd']:.1f}%")

    best_ra_sl = max(SL_FRACTIONS, key=lambda sl: all_stats[sl]["ret"] / all_stats[sl]["dd"] if all_stats[sl]["dd"] > 0 else 0)
    best_ra = all_stats[best_ra_sl]
    if best_ra_sl != best_sl:
        ra = best_ra["ret"] / best_ra["dd"] if best_ra["dd"] > 0 else 0
        print(f"  BEST RISK-ADJUSTED: {best_ra_sl}R SL (Return/MaxDD = {ra:.2f})")
        print(f"    Final: ${best_ra['final']:,.2f} ({best_ra['ret']:+.1f}%) | WR: {best_ra['wr']:.1f}% | PF: {best_ra['pf']:.2f} | DD: {best_ra['dd']:.1f}%")

    # Best WR margin over breakeven
    best_margin_sl = max(SL_FRACTIONS, key=lambda sl: all_stats[sl]["wr"] - breakeven_wrs[sl])
    bm = all_stats[best_margin_sl]
    margin = bm["wr"] - breakeven_wrs[best_margin_sl]
    print(f"  BEST WR MARGIN: {best_margin_sl}R SL (WR {bm['wr']:.1f}% vs BE {breakeven_wrs[best_margin_sl]:.1f}% = +{margin:.1f}pp)")

    # ─── Per-Asset Breakdown for best scenario ──────────────────────────────
    best_r = results[best_sl]
    print_asset_breakdown(f"Best: {best_sl}R SL", best_r["signals"])
