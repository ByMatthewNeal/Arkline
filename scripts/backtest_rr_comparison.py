#!/usr/bin/env python3
"""
Compare MIN_RR_RATIO = 1.0 vs 0.75 over the past 7 days.
Uses same logic as backtest_recent_5d.py but runs both thresholds.
"""
...
def compute_score(zone, is_buy, rr):
    ...
    else: score += 2  # sub-1.0 R:R gets minimal points
    score += 10  # macro base
...
def run_backtest(candles_cache, min_rr):
    ...
    for asset in ASSETS:
        ...
        candles_1h = candles_cache[symbol]["1h"]
        candles_4h = candles_cache[symbol]["4h"]
...
def print_results(label, all_signals):
    print(f"\n{'='*130}")
    print(f"  {label} — Last {LOOKBACK_DAYS} Days")
    print(f"{'='*130}")
    ...
    pf = (sum(s.outcome_pct for _, s in all_signals if s.outcome == "win") /
          abs(sum(s.outcome_pct for _, s in all_signals if s.outcome == "loss"))) if losses > 0 else float('inf')
    print(f"\n  {total} closed | {wins}W / {losses}L ({wr:.1f}% WR) | P&L: {total_pnl:+.2f}% | PF: {pf:.2f}")
    still_open = sum(1 for _, s in all_signals if s.status == "triggered")
    if still_open:
        print(f"  {still_open} signals still open")

    # Show extra signals (ones only in 0.75 tier)
    return total, wins, losses, total_pnl, len(all_signals)


def main():
    # Fetch candles once, reuse for both runs
    candles_cache = {}
    ...
    for asset in ASSETS:
        ...
        time.sleep(0.3)
        candles_cache[symbol] = {"1h": candles_1h, "4h": candles_4h}
        print(f"{len(candles_1h)} 1H / {len(candles_4h)} 4H candles")

    # Run both thresholds
    signals_100 = run_backtest(candles_cache, min_rr=1.0)
    signals_075 = run_backtest(candles_cache, min_rr=0.75)

    print_results("MIN R:R = 1.0 (old)", signals_100)
    print_results("MIN R:R = 0.75 (new)", signals_075)

    # Show the extra signals that 0.75 adds
    old_keys = set(f"{t}_{s.entry_time}_{s.entry_mid}" for t, s in signals_100)
    new_only = [(t, s) for t, s in signals_075 if f"{t}_{s.entry_time}_{s.entry_mid}" not in old_keys]

    if new_only:
        print(f"\n{'='*130}")
        print(f"  NEW SIGNALS (only with R:R >= 0.75, filtered at 1.0)")
        print(f"{'='*130}")
        print(f"{'Time (UTC)':<18} {'Asset':<7} {'Type':<12} {'Grade':>5} {'Entry':>10} {'T1':>10} {'SL':>10} {'R:R':>5} {'Outcome':>8} {'PnL':>8} {'Dur':>5}")
        print("─" * 130)
        new_wins = 0
        ...
        for ticker, sig in new_only:
            ...
            pnl = f"{sig.outcome_pct:+.2f}%" if sig.outcome_pct else "---"
            dur = f"{sig.duration_hours}h" if sig.duration_hours else "---"
            if sig.outcome == "win": new_wins += 1; new_pnl += sig.outcome_pct
            elif sig.outcome == "loss": new_losses += 1; new_pnl += sig.outcome_pct
            print(f"{time_str:<18} {ticker:<7} {typ:<12} {g:>5} {entry:>10} {t1:>10} {sl:>10} {sig.rr_ratio:>5.1f} {outcome:>8} {pnl:>8} {dur:>5}")
        print("─" * 130)
        nt = new_wins + new_losses
        nwr = (new_wins / nt * 100) if nt > 0 else 0
        print(f"\n  {len(new_only)} extra signals | {new_wins}W / {new_losses}L ({nwr:.1f}% WR) | P&L: {new_pnl:+.2f}%")
    else:
        print("\n  No extra signals from lowering R:R to 0.75 in this period.")
