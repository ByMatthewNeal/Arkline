#!/usr/bin/env python3
"""
Backtest: Fibonacci 0.618/0.786 — 1h Entry / 4h Bias
...
# Timeframes: 1h for entry, 4h for higher-TF confluence
TIMEFRAME_CONFIGS = [
    {"tf": "1h", "interval": "1h", "candles_needed": 2160},    # 90 days
    {"tf": "4h", "interval": "4h", "candles_needed": 540},     # 90 days
]
...
CONFLUENCE_TOLERANCE_PCT = 1.0   # Clustering tolerance for 1h
SIGNAL_PROXIMITY_PCT = 1.5      # Price within 1.5% of zone
MIN_RR_RATIO = 1.0
STRONG_MIN_RR_RATIO = 2.0
STRONG_MIN_CONFLUENCE = 2
SIGNAL_EXPIRY_BARS = 12          # 12 x 1h = 12 hours
WICK_REJECTION_RATIO = 1.5
VOLUME_SPIKE_RATIO = 1.3

BACKTEST_DAYS = 90
WARMUP_BARS = 50  # 50 x 1h = ~2 days warmup

# Trading session: 8am - 1pm EST (13:00 - 18:00 UTC)
SESSION_START_UTC = 13  # 8am EST = 13:00 UTC
SESSION_END_UTC = 18    # 1pm EST = 18:00 UTC
...
# ─── Fetch Data (with pagination for 15m) ────────────────────────────────────

def fetch_klines_paginated(symbol: str, interval: str, total_needed: int) -> list[Candle]:
    """Fetch up to total_needed candles, paginating backward from now."""
    all_candles = []
    end_time = None
    ...
    while remaining > 0:
        ...
        # Next page: end before the earliest candle we got
...
def cluster_levels(levels: list[FibLevel], current_price: float) -> list[dict]:
    ...
    nearby = [l for l in levels if abs((l.price - current_price) / current_price) * 100 <= 10]
    if not nearby:
        return []
    ...
    for i in range(1, len(nearby)):
        ...
        if distance_pct <= CONFLUENCE_TOLERANCE_PCT:
            ...
            if len(current_cluster) >= 2:
                ...
                # Count unique timeframes for stronger confluence
...
def calc_ema(candles: list[Candle], period: int) -> float | None:
    """Calculate EMA of closing prices. Returns the final EMA value."""
...
def check_trend_alignment(candles_15m: list[Candle], is_buy: bool) -> bool:
    """
    Check if the short-term trend aligns with the trade direction.
    Uses 20 EMA vs 50 EMA on the 15m chart:
      - BUY: 20 EMA > 50 EMA (uptrend) OR price pulling back to rising 50 EMA
      - SELL: 20 EMA < 50 EMA (downtrend) OR price pushing up to falling 50 EMA
    Also checks EMA slope over last 4 candles (1 hour) for momentum confirmation.
    """
    if len(candles_15m) < 55:
        return True  # Not enough data, allow signal

    ema_fast = calc_ema(candles_15m, 20)
    ema_slow = calc_ema(candles_15m, 50)

    # Also compute EMA slow from 4 candles ago to check slope
    ema_slow_prev = calc_ema(candles_15m[:-4], 50)
    ...
    if is_buy:
        # Allow buy if: fast > slow (uptrend) OR price near rising slow EMA (pullback buy)
        trend_ok = ema_fast > ema_slow
        pullback_ok = ema_slope_up and abs(price - ema_slow) / ema_slow < 0.005
        return trend_ok or pullback_ok
    else:
        # Allow sell if: fast < slow (downtrend) OR price near falling slow EMA (pullback sell)
        trend_ok = ema_fast < ema_slow
        pullback_ok = ema_slope_down and abs(price - ema_slow) / ema_slow < 0.005
        return trend_ok or pullback_ok


# ─── Bounce Confirmation (on 15m candles, approximating 1m entry) ─────────────
...
# ─── Target & Stop (tighter for intraday) ────────────────────────────────────
...
def compute_targets_and_stop(zone: dict, all_fib_prices: list[float], is_buy: bool):
    ...
    if is_buy:
        ...
        stop_loss = next_down * 0.998 if next_down else zone_mid * 0.99  # Tighter: 0.2% below next level

        levels_above = [p for p in all_prices if p > zone["high"]]
        target1 = levels_above[0] if levels_above else zone_mid * 1.02
        target2 = levels_above[1] if len(levels_above) > 1 else target1 * 1.01
    else:
        levels_above = [p for p in all_prices if p > zone["high"]]
        next_up = levels_above[0] if levels_above else None
        stop_loss = next_up * 1.002 if next_up else zone_mid * 1.01

        levels_below = [p for p in all_prices if p < zone["low"]]
        target1 = levels_below[-1] if levels_below else zone_mid * 0.98
        target2 = levels_below[-2] if len(levels_below) > 1 else target1 * 0.99
...
def _resolve_signal(s: Signal, candle: Candle, eval_time: datetime):
    """
    Resolve a signal using split exit strategy:
      - Before T1: full position, original SL
      - At T1: close 50%, move stop to breakeven, start trailing
      - After T1: trail runner stop at 1R behind best price
      - Runner closes when trail stop is hit or signal expires
    Combined P&L = average of both halves.
    """
    ...
    if is_buy:
        # --- LONG ---
        if not s.t1_hit:
            # Phase 1: Full position, check SL and T1
...
def generate_signals(all_data: dict) -> list[Signal]:
    ...
    for ticker in ASSETS.values():
        candles_1h = all_data[ticker].get("1h", [])
        candles_4h = all_data[ticker].get("4h", [])

        if len(candles_1h) < WARMUP_BARS + 100:
            print(f"  Skipping {ticker}: only {len(candles_1h)} 1h candles")
            continue

        print(f"  Processing {ticker}: {len(candles_1h)} bars...", end="", flush=True)
        ticker_signals = 0

        # Walk forward on 1h bars
        for bar_idx in range(WARMUP_BARS, len(candles_1h)):
            eval_candle = candles_1h[bar_idx]
            eval_time = eval_candle.open_time

            if eval_time < start_date:
                continue

            # Only trade during 8am-1pm EST (13:00-18:00 UTC)
            hour_utc = eval_time.hour
            if hour_utc < SESSION_START_UTC or hour_utc >= SESSION_END_UTC:
                # Still resolve open signals outside session
                for s in signals:
            ...
            history_1h = candles_1h[:bar_idx + 1]
            history_4h = [c for c in candles_4h if c.open_time <= eval_time]

            # Detect swings and compute fibs on 1h and 4h
            all_fib_levels = []
            for tf, history in [("1h", history_1h[-200:]), ("4h", history_4h[-100:])]:
                if len(history) < 30:
                    continue
            ...
            for zone in zones:
                ...
                # EMA trend direction filter — both assets
                if not check_trend_alignment(history_1h, is_buy):
                    continue

                # Bounce confirmation on 1h candles
                recent_1h = history_1h[-25:]
                if not check_bounce(recent_1h, zone["low"], zone["high"], is_buy):
                    continue
                ...
                expires = eval_time + timedelta(minutes=15 * SIGNAL_EXPIRY_BARS)
            ...
            # Resolve open signals on this bar
            for s in signals:
                if s.outcome != "open" or s.asset != ticker:
                    continue
                _resolve_signal(s, eval_candle, eval_time)
...
def simulate_leverage(signals: list[Signal], leverage: int, margin_mode: str) -> dict:
    ...
    for signal in sorted(signals, key=lambda s: s.entry_time):
        ...
        if signal.exit_time and signal.entry_time:
            ...
            hours_held = (SIGNAL_EXPIRY_BARS * 15) / 60
...
def print_results(signals: list[Signal], leverage_results: list[dict]):
    ...
    print(f"\n{'':>6} {'Mode':<6} {'Trades':>6} {'Win%':>6} {'Liq':>4} {'End Bal':>10} "
          f"{'Return':>8} {'MaxDD':>7} {'Fees':>7} {'AvgROI':>8} {'Best':>8} {'Worst':>8}")
    print(f"  {'-'*95}")
    ...
    for r in leverage_results:
        ...
        print(f"  {r['leverage']:>2}x  {m:<6} {r['trades']:>6} {r['win_rate']:>5.1f}% {r['liquidations']:>4} "
              ...
              f"${r['total_fees']:>6.2f} {r['avg_roi']:>+7.1f}% "
              f"{r['best_trade']:>+7.1f}% {r['worst_trade']:>+7.1f}%")

    # Signal log
    print(f"\n{'='*70}")
    print("SIGNAL LOG (times in EST) — 50% at T1 + Trail Runner")
    print(f"{'='*70}")
    print(f"{'DateTime EST':<18} {'Asset':<6} {'Type':<12} {'Entry':>10} {'T1':>10} {'SL':>10} {'R:R':>5} {'Out':<8} {'T1 P&L':>7} {'Runner':>7} {'Total':>7} {'R-Mult':>6} {'Dur'}")
    print(f"{'-'*125}")
    ...
    for s in sorted(signals, key=lambda x: x.entry_time):
        ...
        if s.exit_time and s.entry_time:
            dur_min = int((s.exit_time - s.entry_time).total_seconds() / 60)
            dur_str = f"{dur_min}m" if dur_min < 60 else f"{dur_min // 60}h{dur_min % 60:02d}m"
        else:
            dur_str = "-"

        if s.outcome == "open":
            print(f"{dt_str:<18} {s.asset:<6} {s.signal_type:<12} {s.entry_mid:>10,.2f} {s.target1:>10,.2f} {s.stop_loss:>10,.2f} {s.rr_ratio:>5.1f} {'open':<8} {'...':>7} {'...':>7} {'...':>7} {'...':>6} {dur_str}")
        elif s.t1_hit:
            ...
            r_str = f"{r_mult:+.1f}R"
            print(f"{dt_str:<18} {s.asset:<6} {s.signal_type:<12} {s.entry_mid:>10,.2f} {s.target1:>10,.2f} {s.stop_loss:>10,.2f} {s.rr_ratio:>5.1f} {s.outcome:<8} {t1_str:>7} {run_str:>7} {tot_str:>7} {r_str:>6} {dur_str}")
        else:
            tot_str = f"{s.pnl_pct:+.2f}%"
            r_mult = s.pnl_pct / (s.risk_1r / s.entry_mid * 100) if s.risk_1r > 0 else 0
            r_str = f"{r_mult:+.1f}R"
            print(f"{dt_str:<18} {s.asset:<6} {s.signal_type:<12} {s.entry_mid:>10,.2f} {s.target1:>10,.2f} {s.stop_loss:>10,.2f} {s.rr_ratio:>5.1f} {s.outcome:<8} {'—':>7} {'—':>7} {tot_str:>7} {r_str:>6} {dur_str}")
...
def main():
    print("=" * 70)
    print("FIBONACCI 0.618/0.786 BACKTEST — 1h ENTRY / 4h BIAS")
    print("=" * 70)
    print(f"Period: {BACKTEST_DAYS} days | Assets: {', '.join(ASSETS.values())}")
    print(f"Fibs: 0.618 + 0.786 only | Entry: 1h | Bias: 4h")
    print(f"EMA trend filter: 20/50 on 1h | Session: 8am-1pm EST")
    print(f"Expiry: {SIGNAL_EXPIRY_BARS}h | Proximity: {SIGNAL_PROXIMITY_PCT}%")
    print(f"Strategy: 50% at T1, trail runner with 1R stop")
    print(f"Balance: ${STARTING_BALANCE:,} | Position: {POSITION_SIZE_PCT}%")
    print()
