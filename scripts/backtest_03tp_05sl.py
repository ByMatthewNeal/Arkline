#!/usr/bin/env python3
"""
90-Day Portfolio Backtest: 0.3R TP / 0.5R SL Strategy
...
Risk model:
  ...
  - Stop loss TIGHTENED to 0.5R (half original distance)
  - Take profit at 0.3R (30% of entry-to-T1 distance)
  - Win = +0.3R of risk amount (~+0.6% of portfolio)
  - Loss = -0.5R of risk amount (~-1.0% of portfolio)
  - Breakeven WR ~62.5%

Scenarios:
  1. 0.3R TP / 0.5R SL, MIN_RR = 1.0
  2. 0.3R TP / 0.5R SL, MIN_RR = 1.5

Usage:
    python3 scripts/backtest_03tp_05sl.py
"""
...
SL_FRACTION = 0.5   # 50% of original SL distance
...
# ─── Signal Resolution — 0.3R TP / 0.5R SL ──────────────────────────────────

def resolve_signal_03tp_05sl(signal, candle, candle_time):
    """
    Modified resolution:
      - Take profit at 0.3R (30% of entry-to-T1 distance)
      - Stop loss at 0.5R (50% of original SL distance)
      - Position sized on ORIGINAL 1R SL distance
      - Win P&L = position_size * TP_distance (0.3R worth)
      - Loss P&L = position_size * new_SL_distance (0.5R worth)
    """
    ...
    if is_buy:
        ...
        # Tighten SL to 0.5 of original distance
        original_sl_dist = entry_mid - original_sl  # positive for longs
        new_sl = entry_mid - SL_FRACTION * original_sl_dist
    else:
        tp_price = entry_mid - TP_FRACTION * (entry_mid - t1)
        # Tighten SL to 0.5 of original distance
        original_sl_dist = original_sl - entry_mid  # positive for shorts
        new_sl = entry_mid + SL_FRACTION * original_sl_dist
    ...
    if is_buy:
        # Check tightened SL first (assume SL hit before TP on same candle)
        ...
        for asset in ASSETS:
            ...
            for tier_name in ["scalp", "swing"]:
                ...
                for zone in zones:
                    ...
                    # Position sizing based on ORIGINAL 1R SL distance
...
def main():
    print("=" * 120)
    print("90-DAY PORTFOLIO BACKTEST: 0.3R TP / 0.5R SL STRATEGY")
    ...
    print(f"  SL = {SL_FRACTION:.0%} of original SL distance (0.5R)")
    print("  Position sized on original 1R SL")
    print("  Win = ~+0.6% of portfolio, Loss = ~-1.0% of portfolio")
    print("  Breakeven WR = ~62.5%")
    print()
    print("Scenarios:")
    print("  1. 0.3R TP / 0.5R SL, MIN_RR = 1.0")
    print("  2. 0.3R TP / 0.5R SL, MIN_RR = 1.5")
    print("=" * 120)
    ...
    # ─── Scenario 1: 0.3R TP / 0.5R SL, MIN_RR = 1.0 ──────────────────────
    print("\n" + "=" * 60)
    print("Running Scenario 1: 0.3R TP / 0.5R SL, MIN_RR = 1.0...")
    print("=" * 60)
    signals_1, monthly_1, final_1, dd_1 = run_portfolio_backtest(
        candles_cache, min_rr=1.0, resolve_fn=resolve_signal_03tp_05sl
    )
    closed_1 = [s for s in signals_1 if s.status == "closed"]
    print(f"  Complete: {len(closed_1)} closed signals, final value: ${final_1:,.2f}")

    # ─── Scenario 2: 0.3R TP / 0.5R SL, MIN_RR = 1.5 ──────────────────────
    print("\n" + "=" * 60)
    print("Running Scenario 2: 0.3R TP / 0.5R SL, MIN_RR = 1.5...")
    print("=" * 60)
    signals_2, monthly_2, final_2, dd_2 = run_portfolio_backtest(
        candles_cache, min_rr=1.5, resolve_fn=resolve_signal_03tp_05sl
    )
    closed_2 = [s for s in signals_2 if s.status == "closed"]
    print(f"  Complete: {len(closed_2)} closed signals, final value: ${final_2:,.2f}")

    # ─── Monthly Tables ──────────────────────────────────────────────────────
    print_monthly_table("Scenario 1: 0.3R TP / 0.5R SL + MIN_RR=1.0", monthly_1, final_1, signals_1)
    print_monthly_table("Scenario 2: 0.3R TP / 0.5R SL + MIN_RR=1.5", monthly_2, final_2, signals_2)
    ...
    scenarios = [
        ("0.3R/0.5R RR=1.0", signals_1, monthly_1, final_1, dd_1),
        ("0.3R/0.5R RR=1.5", signals_2, monthly_2, final_2, dd_2),
    ]
    ...
    print(f"\n\n{'='*120}")
    print("  COMPARISON VS PREVIOUS STRATEGIES")
    print(f"{'='*120}")
    print()
    print(f"  {'Strategy':<35} {'Final $':>10} {'Return':>8} {'WR%':>7} {'PF':>7} {'Signals':>8}")
    print(f"  {'─'*81}")
    print(f"  {'0.3R TP / 1R SL, RR=1.0 (prev)':<35} {'$1,025':>10} {'+2.5%':>8} {'71.9%':>7} {'1.03':>7} {'--':>8}")
    print(f"  {'Full T1 / 1R SL (current sys)':<35} {'$940':>10} {'-6.0%':>8} {'33.3%':>7} {'--':>7} {'--':>8}")

    for name, s in stats:
        label = f"0.3R TP / 0.5R SL, {name.split()[-1]}"
        print(f"  {label:<35} ${s['final']:>9,.2f} {s['ret']:>+7.1f}% {s['wr']:>6.1f}% {s['pf']:>7.2f} {s['total']:>8}")
    ...
    # ─── Per-Asset Breakdown for Both Scenarios ─────────────────────────────
