#!/usr/bin/env python3
"""
Max-Range Portfolio Backtest: 0.3R TP — 0.8R SL vs 1.0R SL
Focus 5 assets: BTC, ETH, SOL, SUI, ADA
Both tiers: 1H scalp + 4H swing
Simulated $1,000 portfolio with 2% risk per trade

Fetches maximum available Coinbase data (365 days requested, ~187 days returned).
Compares 0.8R SL (winner) vs 1.0R SL (baseline).

Usage:
    python3 scripts/backtest_08sl_maxrange.py
"""
...
# TP fraction (same for both scenarios)
TP_FRACTION = 0.3   # 30% of entry-to-T1 distance

# SL fractions to compare
SL_FRACTIONS = [0.8, 1.0]
...
def generate_signals(candles_cache, actual_backtest_days):
    ...
    Uses actual_backtest_days based on available data.
    """
...
def run_portfolio_backtest(candles_cache, signal_templates, sl_fraction, actual_backtest_days):
    """
...
def main():
    print("=" * 120)
    print("MAX-RANGE PORTFOLIO BACKTEST: 0.3R TP — 0.8R SL vs 1.0R SL")
    ...
    print(f"  SL scenarios: {', '.join(f'{sl}R' for sl in SL_FRACTIONS)}")
    ...
    if ref_candles:
        ...
        # Use all available data minus warmup
        actual_backtest_days = max(actual_span - 3, 90)  # subtract a few days for warmup
    ...
    for sl_frac in SL_FRACTIONS:
        ...
        signals, monthly, final_val, dd = run_portfolio_backtest(
            candles_cache, signal_templates, sl_frac, actual_backtest_days
        )
    ...
    # Collect all months across both scenarios
    all_months = set()
    for sl_frac in SL_FRACTIONS:
        all_months.update(results[sl_frac]["monthly"].keys())
    sorted_months = sorted(all_months)

    header = f"  {'Month':<10}"
    for sl_frac in SL_FRACTIONS:
        header += f" | {'Start':>10} {'End':>10} {'P&L':>10} {'Sigs':>5} {'W':>4} {'L':>4} {'WR%':>7}"
    print(header)
    sep = f"  {'─' * (10 + len(SL_FRACTIONS) * 60)}"
    print(sep)

    # Print column headers for SL scenarios
    sub_header = f"  {'':>10}"
    for sl_frac in SL_FRACTIONS:
        sub_header += f" |{'── ' + str(sl_frac) + 'R SL ' + '─' * 44}"
    print(sub_header)
    print(sep)
    ...
    for mk in sorted_months:
        row = f"  {mk:<10}"
        for sl_frac in SL_FRACTIONS:
            md = results[sl_frac]["monthly"].get(mk, {
                "start_value": 0, "end_value": 0, "pnl_dollars": 0,
                "signals": 0, "wins": 0, "losses": 0
            })
            wr = (md["wins"] / md["signals"] * 100) if md["signals"] > 0 else 0
            row += f" | ${md['start_value']:>9,.2f} ${md['end_value']:>9,.2f} ${md['pnl_dollars']:>+9.2f} {md['signals']:>5} {md['wins']:>4} {md['losses']:>4} {wr:>6.1f}%"
        print(row)
    ...
    for sl_frac in SL_FRACTIONS:
        ...
        total_pnl = r["final"] - STARTING_CAPITAL
        row += f" | ${STARTING_CAPITAL:>9,.2f} ${r['final']:>9,.2f} ${total_pnl:>+9.2f} {s['total']:>5} {s['wins']:>4} {s['losses']:>4} {s['wr']:>6.1f}%"
    print(row)
    ...
    for sl_frac in SL_FRACTIONS:
        r = results[sl_frac]
        all_stats[sl_frac] = compute_stats(r["signals"], r["final"], r["dd"])
    ...
    detail_rows = [
        ...
        ("Breakeven WR", None),  # special handling
        ("WR Margin", None),     # special handling
    ...
    for label, fn in detail_rows:
        ...
        if label == "Breakeven WR":
            for sl_frac in SL_FRACTIONS:
                print(f" {f'{breakeven_wrs[sl_frac]:.1f}%':>14}", end="")
        elif label == "WR Margin":
            for sl_frac in SL_FRACTIONS:
                margin = all_stats[sl_frac]["wr"] - breakeven_wrs[sl_frac]
                print(f" {f'{margin:+.1f}pp':>14}", end="")
        else:
            for sl_frac in SL_FRACTIONS:
    ...
    for sl_frac in SL_FRACTIONS:
        s = all_stats[sl_frac]
        be = breakeven_wrs[sl_frac]
        margin = s["wr"] - be
        ra = s["ret"] / s["dd"] if s["dd"] > 0 else 0
        print(f"  {sl_frac}R SL: ${s['final']:,.2f} ({s['ret']:+.1f}%) | WR: {s['wr']:.1f}% (BE: {be:.1f}%, margin: {margin:+.1f}pp) | PF: {s['pf']:.2f} | MaxDD: {s['dd']:.1f}% | Ret/DD: {ra:.2f}")

    best_sl = max(SL_FRACTIONS, key=lambda sl: all_stats[sl]["final"])
    print(f"\n  WINNER: {best_sl}R SL")

    # ─── Per-Asset Breakdown for 0.8R SL ─────────────────────────────────────
    print_asset_breakdown("0.8R SL", results[0.8]["signals"])
