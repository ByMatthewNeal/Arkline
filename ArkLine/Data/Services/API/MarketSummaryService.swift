...
final class MarketSummaryService {
    ...
    func fetchLatestBriefing() async throws -> MarketSummary {
        // Check client cache — but invalidate if the cached briefing is from a stale day/slot
        if let cached: MarketSummary = APICache.shared.get(Self.cacheKey) {
            let (currentDate, currentSlot) = currentESTSlot()
            let isCurrentSlot = cached.summaryDate == currentDate && cached.slot == currentSlot
            let isFromToday = cached.summaryDate == currentDate
            if isCurrentSlot || isFromToday {
                return cached
            }
            // Cached briefing is from a previous day — clear and re-fetch
